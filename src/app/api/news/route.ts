    ...
    while ((match = itemRegex.exec(xml)) !== null && count < 6) {
      ...
      if (rawTitle) {
        ...
        if (tLower.includes('fomc statement')) {
          title = '【FOMC声明】FRB政策金利発表 ＆ パウエル議長会見';
          summary = '連邦公開市場委員会（FOMC）は政策金利（FF金利）の最新方針を決定。インフレの持続的な2%目標収束と労働市場の安定化に向けた金融政策の調整を公表しました。';
          points = [
            '政策金利（FF金利誘導目標）の最新決定と今後の利下げペース',
            '雇用の下振れリスクとインフレ再燃リスクを両睨みする政策スタンス',
            '今後の追加データ（CPI・雇用統計）に応じた柔軟な利下げ調整'
          ];
          ...
          points = [
            '年内および来年に向けたFOMCメンバーの想定利下げ回数と到達金利',
            '米国のインフレ沈静化と経済成長シナリオ（ソフトランディング見通し）',
            '中長期的な中立金利の水準に対する当局の見解'
          ];
          ...
          points = [
            '投票メンバー間の利下げ幅やタイミングに関する議論の分布',
            '量的引き締め（QT/バランスシート縮小）の進捗状況と流動性環境',
            '金融市場への波及効果と次回会合に向けた注目論点'
          ];
        }
